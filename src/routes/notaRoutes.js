...
const validationNotaUpdate = [
  body('alumno').optional().isMongoId().withMessage('ID de alumno no válido'),
  body('proyecto').optional().isMongoId().withMessage('ID de proyecto no válido'),
  body('profesor').optional().isMongoId().withMessage('ID de profesor no válido'),
  body('calificacion').optional().isFloat({ min: 0, max: 10 }).withMessage('La calificación debe ser un número entre 0 y 10'),
  body('estado').optional().isIn(['apto', 'no apto']).withMessage('El estado debe ser apto o no apto'),
  body('observaciones').optional().isString().withMessage('Observaciones incorrectas').trim()
];
 ...
 *         description: Error del servidor
 */
router.put('/:id', authRequired, requireRole('admin', 'profesor'), validationNotaUpdate, validate, ctrl.updateNota);
